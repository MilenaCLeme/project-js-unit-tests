...
const createMenu = (objetoPassadoPorParametro) => ({
  ...
  consumption: [],
  order: function order(parametro1) {
    let consumo = this.consumption; 
    consumo.push(parametro1);
  },
  pay: function pay() {
    let soma = 0;
    for (let index = 0; index < this.consumption.length; index += 1) {
      let contem = this.consumption[index];
      if (objetoPassadoPorParametro.food[contem] !== undefined) {
        soma += objetoPassadoPorParametro.food[contem];
      }
      if (objetoPassadoPorParametro.drinks[contem] !== undefined) {
        soma += objetoPassadoPorParametro.drinks[contem];
      }
    }
    let acrescidos = soma * 0.1;
    let somaFinal = acrescidos + soma;
    return somaFinal;
  },
});
