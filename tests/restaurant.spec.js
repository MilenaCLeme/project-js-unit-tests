...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    assert.strictEqual(typeof(Object.values(createMenu())), 'object')
    // const objetoRetornado = createMenu(); // Retorno: { fetchMenu: () => {}, ... }
    // ```
    // TESTE 2: Verifique que, dado que a função createMenu foi chamada  o objeto: com`{ food: {}, drink: {} }`, 
    ...
    assert.deepStrictEqual(objetoRetornado3.consumption, ['coxinha', 'agua', 'coxinha'])
    // Agora faça o TESTE 8 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 8: Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
    const meuRestaurante = createMenu({
      food: {'coxinha': 1.00, 'sanduiche': 2.00},
      drinks: {'agua': 1.00, 'cerveja': 6.90}
    });
    ...
    assert.deepStrictEqual(meuRestaurante.pay(), 4.40)
  });
});
